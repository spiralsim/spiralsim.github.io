---
title: Home
layout: home
nav_order: 1
---

Hi! Here's some more personal things I want to share with the world.

## Online Identity

### spiralsim (2024-)
*spiralsim* is a username I use in many places. It derives from [*spiral similarity*](https://en.wikipedia.org/wiki/Spiral_similarity).

### Rainbow Snowflake (2020-)
I made my [rainbow snowflake logo](/assets/favicon.png) using [WeaveSilk](http://weavesilk.com/).

## Blairpath (2019 - 2026)

[Blairpath.org](https://www.blairpath.org) is a website I coded to help the 3000+ students and teachers of my former high school, Montgomery Blair, navigate and survey its ~300 rooms across 3 floors.

## Wikipedia (2020-)

Under the username [User:JeffreyWTong](https://en.wikipedia.org/wiki/User:JeffreyWTong), I've contributed 1 article and 160+ edits.

## Tetris (2022 - 2024)

Under the username [spiralsim](https://ch.tetr.io/u/spiralsim), at my peak ranking, I was ranked in the top 200 globally in Tetra League (a ranked 1v1 league).
